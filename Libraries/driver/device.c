...
void  Mcu_Io_Init(void)
{
  ...
  GPIO_Init(_4G_POWER_PORT	, &GPIO_InitStructure);
  	GPIO_SetBits(_4G_POWER_PORT,_4G_POWER);
	
  GPIO_InitStructure.GPIO_Pins =_BLE_RET;  
  GPIO_InitStructure.GPIO_MaxSpeed = GPIO_MaxSpeed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT_PP;
  GPIO_Init(_BLE_RET_PORT	, &GPIO_InitStructure);
  //GPIO_ResetBits(_4G_POWER_PORT,_4G_POWER);
	_4G_RET_OFF;	//复位

...
void uart_recvdispose(void)
{
    ...
    {
        Uart2RecvControl.count ++;
        if(Uart2RecvControl.count > CM_TIME_50_MSEC)
        {
            Uart2RecvControl.count = 0;
            uart_service_dwinrecv_send_disp(0,0,Uart2RecvControl.recv_index,Uart2RecvControl.recv_buf);
            Uart2RecvControl.recv_index = 0;
        }
    }
#if(USE_BLE==1)
    if(Uart3RecvControl.recv_index > 0)   //蓝牙说明有数据
    {
        Uart3RecvControl.count ++;
        if(Uart3RecvControl.count > CM_TIME_50_MSEC)
        {
            Uart3RecvControl.count = 0;
            uart_service_send_to_blesend(0,0,Uart3RecvControl.recv_index,Uart3RecvControl.recv_buf);
...
//        }
    ...
    {
        Uart1RecvControl.count ++;
        if(Uart1RecvControl.count > CM_TIME_50_MSEC)
        {
            Uart3RecvControl.count = 0;
            uart_service_4GUart_send_recv(0,0,Uart1RecvControl.recv_index,Uart1RecvControl.recv_buf);

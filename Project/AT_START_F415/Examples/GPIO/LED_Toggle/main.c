...
CPU_STK  AppTaskBLEStk[APP_CFG_TASK_BLE_STK_SIZE];
#endif
...
//    Math_Init();                                                /* Initialize mathematical module                       */
    ...
    if(userdog)
	{
	   IWDG_Config(IWDG_Psc_256 ,625*3);	   //3s   开启看门狗
	}
...
//                 (OS_ERR       *)&err);
    ...
    OSTaskCreate((OS_TCB       *)&AppTaskBLETCB,
                 (CPU_CHAR     *)"App BLE",
                 (OS_TASK_PTR   )AppTaskBELLink,
                 (void         *)0,
                 (OS_PRIO       )APP_CFG_TASK_BLE_PRIO,
                 (CPU_STK      *)&AppTaskBLEStk[0],
                 (CPU_STK_SIZE  )APP_CFG_TASK_BLE_STK_SIZE / 10,
                 (CPU_STK_SIZE  )APP_CFG_TASK_BLE_STK_SIZE,
                 (OS_MSG_QTY    )20,
                 (OS_TICK       )0,
                 (void         *)0,
                 (OS_OPT        )(OS_OPT_TASK_STK_CHK | OS_OPT_TASK_STK_CLR),

...
{
    stChTcb.stGunStat.PWMcycle = ch_get_cp_volt(&stChTcb.stGunStat.uiCpVolt);
...
#if(USE_BLE==1)
            mq_service_ch_send_ble(CH_TO_DIP_STARTFAIL ,UNPLUG ,0,NULL);  //枪未插
#endif
        }
    }
    else
...
void HLW8112DataDispose(void)
{
    ...
    stChTcb.stHLW8112.usLCurrent = F_AC_I_B*1000;
}
...
static void HLW8112_Dispose(void)
{
    ...
    if(F_AC_I > 1.0)
    {
        if(kwh == F_AC_E)
        {
            if((nowSysTime >= lastSysTime) ? ((nowSysTime - lastSysTime) >= (CM_TIME_60_SEC)) : \
                    ((nowSysTime + (0xffffffff - lastSysTime)) >=  (CM_TIME_60_SEC)) )
            {

...
typedef struct
{
	uint16_t	usLCurrent;				//漏电流 0.001A
...
__packed typedef struct
{
	...
	//66需要上传
	 uint16_t	Curkwh[48];				//每个时间段的Kkwh  三位小数
...
typedef struct
{
	FunctionalState Reserva;			//ENABLE 表示蓝牙预约   DISABLE 表示蓝牙非预约
	//预约充电，最多三个时间段
	int8_t curtimequau;		//当前使用时间段  默认为 0xff  
	int8_t num;				//时间段个数
	_RESERVA_TIME time[3];		//预约时间
}_BLE_INFO;
typedef struct
{
    uint8_t                   ucState;
...
uint8_t ch_get_cp_volt(uint32_t *puiCpVolt);

...
/* Private typedef----------------------------------------------------------------------------*/
/*static */
_SIM7600_CMD_STATE   SIM7600CmdState = {0};
...
uint8_t Module_HTTPDownload(_HTTP_INFO *info)
{
    ...
    while(!SIM7600CmdState.HTTP_ReadDone)
    {
        ...
        if((pcontrol->AllLen - down_len) > (URART_4GRECV_LEN - 80))
        {
            Send_AT_READ((URART_4GRECV_LEN - 80));   //一次一次读取
        }
        else
        {
        ...
        if((downtime - lastdowntime)  > (CM_TIME_60_SEC * 3))     //单次下载数据少  3分钟没又下载完成说明超时了
        {
...
uint8_t SIM7600Reset()
{
    ...
    if(++rest > 5)
    {
        if(stChTcb.ucState != CHARGING)    //反复连不上而且不在充电中，就复位
        {
            SystemReset();			//软件复位
        }
        else

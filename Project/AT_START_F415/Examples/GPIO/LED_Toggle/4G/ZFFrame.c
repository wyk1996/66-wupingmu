...
{
    uint8_t sendbuf[20];
    uint8_t * pdevnum = DisSysConfigInfo.ZFDevNum;
...
static uint8_t   ZF_SendDevState(void)
{
    ...
    if( CHARGING == stChTcb.ucState)
    {
        ...
        //充电时间			2字节
        data16 = (cp56time(NULL) - stChTcb.stCh.timestart) / 60;

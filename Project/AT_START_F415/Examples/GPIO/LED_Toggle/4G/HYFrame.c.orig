...
__packed typedef struct {
    uint8_t    equipment_id[16];		//充电桩编码
	uint8_t		UpdataState;			//升级状态   0x00-成功 0x01-编号错误 0x02-程序与桩型号不符 0x03-下载更新文件超时
} hy_cmd_1102 ;
//发送数据
...
uint8_t   HYFreamSend(uint16_t cmd,uint8_t *pdata, uint16_t len)
{
    ...
    printf("tx len:%d,tx data:",15+len);
    for(i = 0; i < 15+len; i++)
    {
...
* Contributor  : 2021年3月19日
*****************************************************************************/
static uint8_t   HY_SendUpdataAck(void)
{
...
//} hy_cmd_1102 ;
    ...
    SendHYCmd1102.UpdataState = 0;			//离线允许充电
...
uint8_t   HY_SendDevStateA(void)
{
	...
	else
	{
        ...
        SendHYCmd104.cum_charge_kwh_amount = stChTcb.stCh.uiChargeEnergy / 10;			//累计充电电量
...
//	uint8_t		downloadtime;	//下载超时时间 min
	...
	length = snprintf(HttpInfo.ServerPassword, sizeof(HttpInfo.ServerAdd), "%s:%s",RecvHYCmd1101.usename,RecvHYCmd1101.password);
    if (length == -1)
    {
        printf("snprintf error, the len is %d", length);
        return FALSE;

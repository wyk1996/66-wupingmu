...
#define dwin_showversion "63B23"    //当前显示版本号，只显示5个字节(升级成功后上传的5个字节)
#define userdog   1          //  1使用看门狗    0不使用看门狗
...
__packed typedef struct
{
    ...
    //uint32_t admincode;  //存储管理员密码
	  uint8_t	ZFDevNum[16];				//桩编号
...
uint8_t  show_Notcalculated(void); //停止时，未结算页面,停留到结算界面
extern uint8_t Clear_flag(void);
uint8_t Set_judge_rete_info(uint32_t price);
extern uint8_t Billbuf[200];
extern uint32_t	uifsChargeEnergy[48];
#endif

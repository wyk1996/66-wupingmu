...
uint32_t  failfunc = 0;  //进入硬件故障之前函数地址
...
uint32_t	uifsChargeEnergy[48];		//每个时间段的充电总费用
uint8_t Billbuf[200] = {0};			//缓冲
 ...
 * @note
 */
uint8_t WriterFmBill(uint8_t billflag)
{
//第一个字节表示时候已经结算     				  billflag:0表示已经结算			billflag：表示未结算
    ...
    if(billflag == 0)
    {
        fal_partition_write(ONLINE_BILL,0,&Billbuf[0],1);
    }
    else
    {
        fal_partition_write(ONLINE_BILL,0,&Billbuf[0],200);
    }
...
{
    if(fal_partition_read(ONLINE_BILL,0,&Billbuf[0],200) < 0)
    {
    ...
    if(Billbuf[0] == 1)
    {
        APP_SetResendBillState(1);
    }
    else {

        APP_SetResendBillState(0);
    }
...
*****************************************************************************/
uint8_t  APPTransactionrecord(uint8_t Stop_reason,_SHOW_NUM Charg_mode,_SHOW_NUM GUNnum,uint32_t CurNum)
{
    int length;
    uint8_t * pbuf;
    ...
    if(DisSysConfigInfo.standaloneornet == DISP_NET)
    {
        if(_4G_GetStartType() == _4G_APP_CARD)
        {
...
uint8_t Munu13_ShowSysInfo(void)
{
    ...
    PrintStr(0x2680,DisSysConfigInfo.ZFDevNum,16);//显示政府编号
...
//    return 1;
    ...
    for(i = 2; i < 14; i++)
    {
        if(pvalue[i-2] >= 0x30 && pvalue[i-2] <= 0x39)
        {
            DisSysConfigInfo.DevNum[i] = pvalue[i-2];
        }
        else
        {
            break;
    ...
    DisSysConfigInfo.DevNum[8] = 0;
    DisSysConfigInfo.DevNum[9] = 0;
    DisSysConfigInfo.DevNum[10] = 0;
    DisSysConfigInfo.DevNum[11] = 0;
    DisSysConfigInfo.DevNum[12] = 0;
    DisSysConfigInfo.DevNum[13] = 0;

    memcpy(&DisSysConfigInfo.DevNum[0],"AC",2);

    fal_partition_write(DWIN_INFO,0,(uint8_t*)&DisSysConfigInfo,sizeof(_DIS_SYS_CONFIG_INFO));
    Munu13_ShowSysInfo();
    OSTimeDly(5, OS_OPT_TIME_PERIODIC, &timeerr);
    PrintStr(0x1050,DisSysConfigInfo.DevNum,8);
    return 1;

}

//====政府编号设置====================
static uint8_t Munu13_ZFDevnumDispose(uint16_t addr,uint8_t *pvalue,uint8_t len)
{
//    uint8_t i;
...
// memcpy(DisSysConfigInfo.ZFDevNum,"AC6601202366153",strlen("AC6601202366153"));  //===政府编号
    ...
    for (i = 0; i <16; i++)
    {
        DisSysConfigInfo.ZFDevNum[i]  = 0x30;
    }
    if(len > 16)
    {
        len = 16;		//ASICC输入时，可能存在len大于16
    ...
    for(i = 2; i < 16; i++)
    {
        if(pvalue[i-2] >= 0x30 && pvalue[i-2] <= 0x39)
        {
            DisSysConfigInfo.ZFDevNum[i] = pvalue[i-2];
        }
        else
        {
            break;
    ...
    //DisSysConfigInfo.ZFDevNum[15] = 0;

    memcpy(&DisSysConfigInfo.ZFDevNum[0],"AC",2);


    //下面是66平台桩编号赋值上面的政府平台的6位桩编号
    memcpy(&DisSysConfigInfo.DevNum[0],"AC",2);
    DisSysConfigInfo.DevNum[2] = DisSysConfigInfo.ZFDevNum[10];
    DisSysConfigInfo.DevNum[3] = DisSysConfigInfo.ZFDevNum[11];
    DisSysConfigInfo.DevNum[4] = DisSysConfigInfo.ZFDevNum[12];
    DisSysConfigInfo.DevNum[5] = DisSysConfigInfo.ZFDevNum[13];
    DisSysConfigInfo.DevNum[6] = DisSysConfigInfo.ZFDevNum[14];
    DisSysConfigInfo.DevNum[7] = DisSysConfigInfo.ZFDevNum[15];
	DisSysConfigInfo.DevNum[8] = 0;  //必须赋值一下0不显示
    DisSysConfigInfo.DevNum[9] = 0;
    DisSysConfigInfo.DevNum[10] = 0;
    DisSysConfigInfo.DevNum[11] = 0;
    DisSysConfigInfo.DevNum[12] = 0;
    ...
    Munu13_ShowSysInfo();
    OSTimeDly(5, OS_OPT_TIME_PERIODIC, &timeerr);
    PrintStr(0x2680,DisSysConfigInfo.ZFDevNum,16);
    return 1;
}
...
static uint8_t Munu13_PortSert(uint16_t addr,uint8_t *pvalue,uint8_t len)
{
    ...
    NetConfigInfo.port= temp; //==hycsh当前某一个协议，对应的端口
...
static uint8_t Munu13_IP1Sert(uint16_t addr,uint8_t *pvalue,uint8_t len)
{
    ...
    snprintf(NetConfigInfo.pIp,sizeof(NetConfigInfo.pIp),"%d.%d.%d.%d",\
...
{
    stChTcb.stCh.Reservetime = cp56time(NULL); //记录开始预约充电时的时间（距1970年秒数）
...
const _DISP_ADDR_FRAME Disp_RecvFrameTable[] =
{
    ...
    {0x2680,Munu13_ZFDevnumDispose}, //政府桩编号
...
static uint8_t Period_WriterFmBill(uint32_t time)
{
    ...
    {
        if(++count >= ((CM_TIME_5_MIN)/time) )   //之前为30s存一下，flash用不了多久，目前临时改成5分钟一次 20210623
            //if(++count >= ((CM_TIME_10_SEC*4)/time) )   //之前为30s存一下，flash用不了多久，目前临时改成5分钟一次 20210623
        {
            ...
            //写入A枪记录
            WriterFmBill(1);
            memcpy((void *)uifsChargeEnergy,(void*)stChTcb.stCh.uifsChargeEnergy,sizeof(uifsChargeEnergy) );
            fal_partition_write(CHARGE_PRIC,0,(uint8_t*)&uifsChargeEnergy,sizeof(uifsChargeEnergy));
...
uint8_t APP_DisplayRecvDataAnalyze(uint8_t *pdata, uint8_t len)
{
    ...
    //临时配置
    if(len > strlen("set pileId:"))
    {
        if(strncmp((char*)pdata,"set pileId:",strlen("set pileId:")) == 0)
        {
            //设置设备号
            if((len - strlen("set pileId:")) > 16)  //最大为16
            {
                UART2SENDBUF((uint8 *)"set pileId fail\r\n",strlen("set pileId fail\r\n"));
                return 0;
            }
            memset(DisSysConfigInfo.DevNum,0,sizeof(DisSysConfigInfo.DevNum));
            memcpy(DisSysConfigInfo.DevNum,&pdata[strlen("set pileId:")],len - strlen("set pileId:"));
            fal_partition_write(DWIN_INFO,0,(uint8_t*)&DisSysConfigInfo,sizeof(_DIS_SYS_CONFIG_INFO));
            UART2SENDBUF((uint8 *)"set pileId success\r\n",strlen("set pileId success\r\n"));

        }
        if(strncmp((char*)pdata,"set ZFpileId:",strlen("set ZFpileId:")) == 0)
        {
            //设置设备号
            if((len - strlen("set ZFpileId:")) > 16)  //最大为16
            {
                UART2SENDBUF((uint8 *)"set ZFpileId fail\r\n",strlen("set ZFpileId fail\r\n"));
                return 0;
            }
            memset(DisSysConfigInfo.ZFDevNum,0,sizeof(DisSysConfigInfo.ZFDevNum));
            memcpy(DisSysConfigInfo.ZFDevNum,&pdata[strlen("set ZFpileId:")],len - strlen("set ZFpileId:"));
            fal_partition_write(DWIN_INFO,0,(uint8_t*)&DisSysConfigInfo,sizeof(_DIS_SYS_CONFIG_INFO));
            UART2SENDBUF((uint8 *)"set ZFpileId success\r\n",strlen("set ZFpileId success\r\n"));
...
static uint8_t DispShow_NetState(void)
{
    ...
    {
        if(	APP_GetAppRegisterState(0))   //网络在线
        {
            state[0] = 0;
...
static void dwin_show(void)
{
    ...
    PrintStr(0x1050,DisSysConfigInfo.DevNum,8); //显示桩编号
    /* 界面显示RTC */
    Disp_ShowRTC();
...
///**
//* @brief 0表示灭   1~13 表示闪烁次数    0xaa表示常量
// * @param[in]
// * @param[out]
// * @return
// * @note
...
static void flash_para_init(void)
{
    ...
    if(fal_partition_read(HardFault,0,(uint8_t*)&sijit123,sizeof(sijit123)) < 0)  //读取死机存储区域
    {
        printf("Partition read error! Flash device(%d) read error!", CHARGE_RATE);
    }
...
//	uint8_t* func=&bufff;
//	sijit123.NUM =0;
//	fal_partition_write(HardFault,4,(uint8_t*)&bufff,20); //
//  fal_partition_write(HardFault,8,(uint8_t*)&bufff,20); //
//  fal_partition_write(HardFault,0,(uint8_t*)&sijit123.NUM,1);//
    ...
    {
        //说明flash没有设置过，为初始值
        DisSysConfigInfo.IP[0]= NetConfigInfo.IP[0];
        DisSysConfigInfo.IP[1]= NetConfigInfo.IP[1];
        DisSysConfigInfo.IP[2]= NetConfigInfo.IP[2];
        DisSysConfigInfo.IP[3]= NetConfigInfo.IP[3];
        //写
        fal_partition_write(DWIN_INFO,0,(uint8_t*)&DisSysConfigInfo,sizeof(_DIS_SYS_CONFIG_INFO));
    ...
    {
        //说明flash没有设置过，为初始值
        DisSysConfigInfo.Port = NetConfigInfo.port;
        fal_partition_write(DWIN_INFO,0,(uint8_t*)&DisSysConfigInfo,sizeof(_DIS_SYS_CONFIG_INFO));

    }
    else
    {
        NetConfigInfo.port = DisSysConfigInfo.Port;
    }
    //第一次烧录默认是单机正常、内部电表
    ...
    fal_partition_read(CHARGE_PRIC,0,(uint8_t*)&uifsChargeEnergy,sizeof(uifsChargeEnergy));
    for(i = 0; i < 48; i++)
    {
        if(uifsChargeEnergy[i] > 100000000)   //大于100元
        {
            break;
        }
    }
    if(i < 48)  //小于48说明有问题
    {
        memset(uifsChargeEnergy,0,sizeof(uifsChargeEnergy));
        fal_partition_write(CHARGE_PRIC,0,(uint8_t*)&uifsChargeEnergy,sizeof(uifsChargeEnergy));
    }
...
//    memset(DisSysConfigInfo.DevNum,0,sizeof(DisSysConfigInfo.DevNum));
//    memset(DisSysConfigInfo.ZFDevNum,0,sizeof(DisSysConfigInfo.ZFDevNum));
//    memcpy(DisSysConfigInfo.DevNum,"AC660153",strlen("AC660153"));     //0419后面的枪号不写

//    uint8_t DevNumbuf[16]= {0};
//    memcpy(DevNumbuf,DisSysConfigInfo.DevNum,8);
//    memcpy(DisSysConfigInfo.DevNum,DevNumbuf,16);

    // memcpy(DisSysConfigInfo.ZFDevNum,"AC6601202366153",strlen("AC6601202366153"));  //===政府编号


    DisSysConfigInfo.standaloneornet = DISP_NET; //网络

    if(USE_BLE ==2)
    {
        DisSysConfigInfo.energymeter = NOUSERN8209;  //默认外电表
    }
    else
    {
        DisSysConfigInfo.energymeter = USERN8209;  //默认内部表
    }


    //66测试环境  交流:39.170.80.51:18992
    //NetConfigInfo.port = 18992;

    //66交流正式环境  交流正式:139.196.60.58:18051
    NetConfigInfo.port = 18051;
}
...
void rest_charge_dispose(void)
{
    ...
    if(ResendBillControl.ResendBillState == TRUE)  //需要发送订单
    {
        if(stChTcb.ucState == INSERT)  //再次启动充电
        {
            APP_SetResendBillState(0);  //先不要发送订单
            ...
            case CM_CHTASK_MODULE_ID:   /* 充电任务模块ID */
                ...
                case CH_TO_DIP_STARTSUCCESS: 	//启动成功
                    ...
                    if(DisSysConfigInfo.standaloneornet == DISP_NET)
                    {
                        ...
                        {
                            fal_partition_write(CHARGE_RATE,0,(uint8_t*)&stChRate,sizeof(RATE_T));  //存费用，断电重启继续充电有用
                            WriterFmBill(1);   //存储交易记录
                ...
                case CH_TO_DIP_STOP:			//停止时
                    ...
                    stChTcb.stCh.uiChStoptTime = gs_SysTime;	//停止充电时间
                    stChTcb.stCh.timestop = cp56time(NULL);
